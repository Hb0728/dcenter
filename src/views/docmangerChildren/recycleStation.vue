<template>
  <div class='recycleStation'>
      <doc_header :headerName="'下架文档（'+firstTotals+'）'" headerTips="下架文档表示发布成功的文档由于违规而被系统下架" @updatepage="updatepage"></doc_header>
    ...
    methods: {
      ...
      getitemlist(dh,db,isfirst){
        ...
        .then(function(res){
            ...
            if(isfirst){
              window.localStorage.setItem('recycleStationTotal', JSON.stringify(res.data.data.total/1))
              _this.firstTotal=res.data.data.total/1
              _this.$store.commit('setrecycleStationTotal', res.data.data.total/1)
            }
        })
